...
<section class="">
    <div class="Hui-article">
        <article class="cl pd-20">
            <form class="form form-horizontal" action="<%=request.getContextPath()%>/position/updatePosition">
                <div class="mt-20">
                    <table class="table table-border  table-hover table-bg ">
                        ...
                        <tr class="text-c">
                            ...
                            <td class="text-l">
                                <h3>职位名称:<input type="text" class="input-com" id="position_name" name="position_name" value="${position.position_name}"></h3>
                                <input type="hidden" name="position_id" value="${position.position_id}">
                                <p >三年工作平均薪资（元/月)
                                    <!--&nbsp;&nbsp;&nbsp;&nbsp;-->
                                    <label class="c-999 f-12" style="padding-left: 20px">
                                        一线 <input type="text"  id="one_line_salary" name="one_line_salary" class="input-com" value="${position.one_line_salary}">
                                        vs  二线
                                        <input type="text"  class="input-com" id="two_line_salary" name="two_line_salary" value="${position.two_line_salary}">
                                    </label>
                                </p>
                                <!--<div class="c-999 f-12">
                                    一线 12313213132  vs  本地 43231 </div>-->
                                <p>需求量（人） <label class="c-999 f-12" style="padding-left: 60px">
                                    社会 <input type="text" name="total_demand" id="position_total_demand"  class="input-com" value="${position.total_demand}">
					...
					<select class="select-overried" size="1" name="city_code" id="city_code" onchange="putValue(this)">
                        <c:forEach items="${position.positionMarketList}" var="positionMarket">
                                    <option value="${positionMarket.city_code}"
                                            alt="${positionMarket.local_demand}_${positionMarket.local_avg_salary}" data-id="${positionMarket.pm_id}">
                                            ${positionMarket.city_name}</option>
                                    <%--<input type="hidden" id="hideen_total_demand_${positionMarket.city_code}" value="${positionMarket.total_demand}">
                                    <input type="hidden" id="hideen_avg_salary_${positionMarket.city_code}" value="${positionMarket.total_avg_salary}">--%>
                        </c:forEach>
                    ...
                    <%--<positionIntroduce--%>
                    <div class="row cl">
                        <input type="hidden" name="id" value="${position.positionIntroduce.id}">
                        <label>名词解释：</label>
                        <div class="formControls">
                            <textarea name="noun_interpretation" id="noun_interpretation" cols="" rows="" class="textarea"  maxlength="1500" placeholder="名词解释"  onKeyUp="textarealength(this,1500)">
                                ${position.positionIntroduce.noun_interpretation}
                            </textarea>
                    ...
                    <div class="row cl">
                        ...
                        <div class="formControls">
                            <textarea name="introduce" id="introduce" cols="" rows=""   maxlength="3000" class="textarea"  placeholder="职位内容" onKeyUp="textarealength(this,3000)">
                                    ${position.positionIntroduce.introduce}
                            </textarea>
                    ...
                    <div class="row cl">
                        ...
                        <div class="formControls">
                            <textarea name="requirement" id="requirement" cols="" rows="" class="textarea"   maxlength="500" placeholder="职位要求" onKeyUp="textarealength(this,500)">
                                    ${position.positionIntroduce.requirement}
                            </textarea>
                    ...
                    <div class="row cl">
                        ...
                        <div class="formControls">
                            <textarea name="direction"  id="direction" cols="" rows="" class="textarea"  maxlength="500" placeholder="晋升空间" onKeyUp="textarealength(this,500)">
                                    ${position.positionIntroduce.direction}
                            </textarea>
                            <p class="textarea-numberbar"><em class="textarea-length">0</em>/500</p>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        */
        ...
        textarealength($("#requirement"),500);
        textarealength($("#noun_interpretation"),1500);
        textarealength($("#introduce"),3000);
        textarealength($("#direction"),500);
        putValue();
    });
    /**
...
//    var tatal = html.find('am').html();
    ...
    /*<span>需求量:<input type="text" class="input-com" id="positionMarket.total_demand">
        ...
        var alt = $("#city_code option:selected").attr("alt");
        var pm_id = $("#city_code option:selected").attr("data-id");
        var city_name = $("#city_code option:selected").text();
        $("#city_name").val($.trim(city_name));
        $("#pm_id").val(pm_id);
        if(alt != null && alt != ""){
            alt = alt.split("_");
        $("#local_demand").val(alt[0]);
        $("#local_avg_salary").val(alt[1]);
        }
    }
    ...
    function formSubmit(){
        ...
        if(city_code != null && city_code!= ''&&local_demand != null && !reg.test(local_demand)){
            layer_show("需求量只能是正整数");
            return ;
        }
        if(one_line_salary != null &&($.trim(one_line_salary) )!= '' && !reg1.test($.trim(one_line_salary))){
            layer_show("薪水（一线）填写错误");
            return;
        }
        if(two_line_salary != null &&($.trim(two_line_salary))!='' && !reg1.test(two_line_salary)){
            layer_show("薪水（二线）填写错误");
            return;
        }
        if(city_code != null && city_code!= ''&&local_avg_salary != null && !reg1.test(local_avg_salary)){
            layer_show("本地平均薪水 填写错误");
            return;
        }
        $("form").ajaxSubmit(function(data){
           layer_show(data.desc);
           top.location.href="<%=request.getContextPath()%>/position/findPositionByKey";
